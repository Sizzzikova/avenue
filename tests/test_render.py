...
class TestCarLine:
    def test_contains_link_prices_and_percent(self):
        line = car_line(car(), "")
        assert 'href="https://avenuerent.ru/autopark/cars/6312/"' in line
        assert "Exeed TXL 4WD" in line
        assert "<s>8 400 ₽</s>" in line
        assert "−20%" in line

    def test_badge_is_prefixed(self):
        assert car_line(car(), "🆕").startswith("🆕 ")

    def test_deadline_shown_when_known(self):
        assert "(до 31.12.2026)" in car_line(car(discount_to="2026-12-31"), "")
...
class TestDigest:
    ...
    def test_cities_follow_config_order(self):
        ...
        caption = build_digest(cars, state.EMPTY, CFG, TODAY)[0].caption
    ...
    def test_caption_never_exceeds_telegram_limit(self):
        cars = [car(car_id=str(i), name=f"Очень Длинное Название Модели {i}")
                for i in range(40)]
        for post in build_digest(cars, state.EMPTY, CFG, TODAY):
    ...
    def test_cars_without_photo_still_appear(self):
        posts = build_digest([car(image_url=None)], state.EMPTY, CFG, TODAY)
        assert posts[0].images == []
        assert "Exeed TXL 4WD" in posts[0].caption

    def test_badges_reflect_previous_snapshot(self):
        previous = state.build([car(car_id="6312", discount_pct=10)])
        cars = [car(car_id="6312", discount_pct=25), car(car_id="5750", name="Chery Tiggo 4 Pro")]
        caption = build_digest(cars, previous, CFG, TODAY)[0].caption
        assert "📉" in caption  # у Exeed скидка выросла
        assert "🆕" in caption  # Chery появился впервые
