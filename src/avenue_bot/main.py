...
def run(args: argparse.Namespace) -> int:
    ...
    if args.mode == "run" and not state.data and all_promos and not args.allow_empty_state:
        alerter.notify(
            ...
            "Скорее всего, не выполнен прогон в режиме seed или в прошлый раз не "
            "сохранился state/seen.json — проверьте шаг «Сохранить состояние» в "
            "последнем прогоне Actions.\n\n"
            "Если это действительно первый запуск и опубликовать всё нужно намеренно, "
            "запустите с флагом --allow-empty-state."
        )
        alerter.close()
        return 1
    ...
    if (
        ...
        alerter.notify(
            ...
            "узнавать сохранённые записи. Проверьте страницу акций и при "
            "необходимости прогоните режим seed заново.\n\n"
            "Если обновился действительно весь автопарк, запустите с флагом "
            "--allow-empty-state."
        )
        alerter.close()
        return 1
